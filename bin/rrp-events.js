...
const args = yargs.usage("\nUsage: rrp-events <event type: full | template | fulfilled | failed | sponsor>")
    ...
    .command("full", "MadeFullRequest", airnodeRequestOptions)
    .command("template", "MadeTemplateRequest", airnodeRequestOptions)
    .command("fulfilled", "FulfilledRequest", airnodeRequestOptions)
    .command("failed", "FailedRequest", airnodeRequestOptions)
    .command("sponsor", "SetSponsorshipStatus", sponsorRequesterOptions)
...
const requests = rrp.queryFilter(filters[args._[0]], args.fromBlock, args.toBlock);
...
requests.then(results => formats[args.output](results.map(maps[args._[0]])));
